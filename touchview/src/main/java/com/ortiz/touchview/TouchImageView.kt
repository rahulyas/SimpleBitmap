...
open class TouchImageView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) :
    ...
    private inner class GestureListener : SimpleOnGestureListener() {
        override fun onSingleTapConfirmed(e: MotionEvent): Boolean {
            ...
            touchImageViewListener?.onSingleTap(e.x, e.y)
    ...
    private inner class PrivateOnTouchListener : OnTouchListener {
        ...
        override fun onTouch(v: View, event: MotionEvent): Boolean {
            ...
            if (imageActionState == ImageActionState.NONE || imageActionState == ImageActionState.DRAG || imageActionState == ImageActionState.FLING) {
                when (event.action) {
                    ...
                    MotionEvent.ACTION_MOVE -> if (imageActionState == ImageActionState.DRAG) {
                        ...
                        fixTrans()
                        last[curr.x] = curr.y
                        touchImageViewListener?.onDrag(deltaX, deltaY)
                    }
           ...
           // touchImageViewListener?.onMove(0f,0f)
    ...
    private inner class ScaleListener : SimpleOnScaleGestureListener() {
        override fun onScaleBegin(detector: ScaleGestureDetector): Boolean {
            ...
            touchImageViewListener?.onScaleBegin( detector.focusX,  detector.focusY, detector.scaleFactor)
            return true
        }
...
//            Log.d("prevSpan", detector.previousSpan.toString())
           ...
           /* Log.d("currentSpanX", detector.currentSpanX.toString())
            Log.d("currentSpanXy", detector.currentSpanY.toString())*/


            // OnTouchImageViewListener is set: TouchImageView pinch zoomed by user.
            touchImageViewListener?.onMove(detector.focusX, detector.focusY,detector.scaleFactor)
            touchImageViewListener?.onSpan(detector.currentSpan, detector.currentSpanY,detector.previousSpan, detector.previousSpanY )
            return true
        }

        override fun onScaleEnd(detector: ScaleGestureDetector) {
            super.onScaleEnd(detector)
            touchImageViewListener?.onScaleEnd( detector.focusX,  detector.focusY, detector.scaleFactor)
            setState(ImageActionState.DRAG)
      /*      setState(ImageActionState.NONE)
